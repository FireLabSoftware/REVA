...
FiD0=[] ##'R1' or 'R2' Indicates that this is specified by user as read1 or read2, r1 or r2 indicates REVAs interpretation
...
FastFindAlgorithm1 = 1 ## Two basically equivalent algorighms for rapid retrieval of id's from a sorted list.  If Speed is critical it may be useful to try FastFindAlgorithm=2 to see if this is faster in your Python/Numpy implementation
for i in range(8):
...
while ai1<len(argv):
    ...
    if not('=' in a1) and (a1.lower().endswith('files') or a1.lower().endswith('files.txt')):
        ...
        for fx1 in R1File1.read().replace(',',' ').split():
            if os.path.isfile(fx1):
                ...
                iD0 = 'R1'
                if R2FileTest1(fx1):
                    if os.path.isfile(fx1.replace('_R2','_R1')) or os.path.isfile(fx1.replace('_r2','_r1')) or os.path.isfile(fx1.replace('_2.','_1.')):
                        iD0 = 'R2'
                    if os.path.isfile(fx1.replace('_R2','_R1',1)) or os.path.isfile(fx1.replace('_r2','_r1',1)) or os.path.isfile(fx1.replace('_2.','_1.',1)):
                        iD0 = 'R2'
                    if os.path.isfile(fx1[::-1].replace('2R_','1R_',1)[::-1]) or os.path.isfile(fx1[::-1].replace('2r_','1r_',1)[::-1]) or os.path.isfile(fx1[::-1].replace('.2_','.1_',1)[::-1]):
                        iD0 = 'R2'
    ...
    if a11.startswith('dfam'):
    ...
    elif a11.startswith('fastfindalgorithm') :
        FastFindAlgorithm1=int(a22)    
        ...
        klen1=int(a22)
        if klen1>32:
            raise ValueError('Kmer length cannot be set beyond 32, it appears that this is being set on the command line to a greater value with:"'+a1+'"'+"; REVA's Suggestion : rerun with klen=31")
...
def sdeviations1(ListN0,ListN1,ListN2):
    ...
    for g0,g1,g2 in zip(ListN0,ListN1,ListN2):
        if g0==0:
            ...
            StD1.append(max(0.0, ( (1.0*g2)/g0-(1.0*g1*g1)/(g0*g0) ))**0.5)
    return StD1
def str02(ListF1):
    FT1=[]
    for r1 in ListF1:
...
if RefSeqFile1.endswith('.gz'):
    ...
    for zfn1 in ZipFile1.namelist():
        if zfn1[0]!='_':
            F0.extend(ZipFile1.open(zfn1,mode='rt').read().splitlines())
else:
    if version.startswith('2.'):
        F0=open(RefSeqFile1,mode='rU')
    else:
        F0=open(RefSeqFile1,mode='rt')
SD1=[] ## Sense Sequences
...
CoverageSEMax2 = CoverageSEMax1-1
if FullCoverage1:
...
def FastFindB1B(aV,N1):
    lastL=FastIndex1[N1>>BitsSorted1]    ## i=lastL-1 is the first position in aV that might hold a matching k-mer
    lastH=FastIndex1[(N1>>BitsSorted1)+1]    ## i=lastH is the first position in aV that could hold a matching k-mer
    N2 = N1 & BitMask1
    mk = np.nonzero(lastL<lastH-1)[0]
    mkk = lastL==lastH
    while len(mk):
        lastM = (lastL[mk]+lastH[mk])>>1
        mkh = N2[mk]<aV[lastM]
        mkl = N2[mk]>=aV[lastM]
        lastH[mk[mkh]] = lastM[mkh]
        lastL[mk[mkl]] = lastM[mkl] 
        mk = mk[lastL[mk]<(lastH[mk]-1)]
    lastL[aV[lastL]!=N2]=ulen1
    lastL[mkk]=ulen1
    return lastL

if int(FastFindAlgorithm1) == 1:
    FastFindB1 = FastFindB1A
else:
    FastFindB1 = FastFindB1B
...
## NOTE the following routine is not currently used, but is preserved for possible
## Future use.  To use this to generate an external database (e.g. wormbase or modern equivalent) link for a
## given dataset, the programmer would need also to add a method that populates GeneNameD1 with conversions between the name
## in the relevant annotation file (e.g. .gtf file) and names in the online reference
## because wormbase is not currently maintainted, this will likely be a successor database if the routine is eventually added
# def Wormbaselink1(gn0):
#     if '.' in gn0:
#         gn1='.'.join(gn0.split('.')[:2])
#     else:
#         gn1=gn0
#     gn2=gn1.rstrip(alphalow1)
#     if gn2 in GeneNameD1: ## GeneNameD1 needs to be populated outside this loop to translate names from the annotation file (e.g. GFF or GTF) to names used in the external database.
#         gn3=GeneNameD1[gn2]
#     else:
#         gn3=gn0
...
def CalculateFullCoverage1(Milestone1):
    ...
    for z in range(NumSeq1):
        ...
        if FullCount1 and (KmerBinCoverageUnique1 or KmerBinCoverageAll1):
            ...
            np.add.at(UniqueKCountS2[z],qxB1,CoverageK1[q1].astype(np.uint64)**2)
            np.add.at(UniqueKCountA2[z],qxB2,CoverageK1[q2].astype(np.uint64)**2)
        ...
        if FullCount1 and KmerBinCoverageAll1:
            ...
            np.add.at(LocalKCountS2[z],qxB1,CoverageK1[q1].astype(np.uint64)**2)
            np.add.at(LocalKCountA2[z],qxB2,CoverageK1[q2].astype(np.uint64)**2)
        ...
        if FullCount1 and KmerBinCoverageAll1:
            ...
            np.add.at(ChromosomalKCountS2[z],qxB1,CoverageK1[q1].astype(np.uint64)**2)
            np.add.at(ChromosomalKCountA2[z],qxB2,CoverageK1[q2].astype(np.uint64)**2)
        ...
        if FullCount1 and KmerBinCoverageAll1:
            ...
            np.add.at(DispersedKCountS2[z],qxB1,CoverageK1[q1].astype(np.uint64)**2)
            np.add.at(DispersedKCountA2[z],qxB2,CoverageK1[q2].astype(np.uint64)**2)
...
for i0,(f0,d0,n0) in enumerate(zip(FiL1,FiD1,FiN1)):
    if d0=='R1':
        rDom1 = 'R1'
    elif  d0=='R2':
        rDom1 = 'R2'
        ...
        if '_R1' in f0 and not('_R2' in f0):
            ...
            if f0.rfind('_R1')>f0.rfind('_R2'):
                ...
                rDom1='R2'
    if rDom1=='R1':
        if len(FiL1)==2 and set(FiD1)==set(['R1','R2']):
            FiLR1 = [f0,]
            FiLR2 = [FiL1[1-i0],]
            FiLN1 = ['',]
            break
        ...
        if not(os.path.isfile(f0r2)):
            f0r2=f0[::-1].replace('.1R','.2R',1)[::-1]
        if not(os.path.isfile(f0r2)):
            f0r2=f0[::-1].replace('.1_','.2_',1)[::-1]
        if not(os.path.isfile(f0r2)):
            f0r2=f0[::-1].replace('.1','.2',1)[::-1]
        if not(os.path.isfile(f0r2)):
            f0r2=f0[::-1].replace('1_','2_',1)[::-1]
    else:
        f0r2=f0[:]
        f0r1=f0[::-1].replace('2R_','2R_',1)[::-1]
        if not(os.path.isfile(f0r1)):
            f0r1=f0[::-1].replace('.2R','.1R',1)[::-1]
        if not(os.path.isfile(f0r2)):
            f0r1=f0[::-1].replace('.2_','.1_',1)[::-1]
        if not(os.path.isfile(f0r2)):
            f0r1=f0[::-1].replace('.2','.1',1)[::-1]
        if not(os.path.isfile(f0r2)):
            f0r1=f0[::-1].replace('2_','1_',1)[::-1]
        ...
        for CurRegionFile1 in BaseByBaseRegionsFiles1:
            if CurRegionFile1.endswith('gz'):
                ...
                if version.startswith('2.'):
                    ...
                    bbFF1 = open(CurRegionFile1,mode='rt')
...
for (f1,f2,n00) in zip(FiLR1,FiLR2,FiLN1):
    ...
    for L0,M0 in izip_longest(F1,F2,fillvalue=''):
        ...
        if (filelinenum & 3 == 2 or (FastAFile1 and (filelinenum & 3 == 0))) and ((GoodSeqIndex1 and SeqIndexMode1<2) or (not(GoodSeqIndex1) and SeqIndexMode1==2)):    ##only pay attention to the second line of each 4 (fastq file structure)
            ...
            if AvoidBothAmbiguous1 and (p1==0 and p2==0): continue
            ...
            if ReportStarts1:
                if p0!=0:
                    CoverageS1[x0]= min(CoverageS1[x0],CoverageSEMax2)+1  ## if there is a unique k-mer assign the start to the start of that k-mer; this results in a potential anomaly where the interface between repeated and unique sequence will serve as the effective start point for any read spanning the junction.  So caveat emptor!
                elif 0<=ix0<ulen1:
                    CoverageS1[ix0]= min(CoverageS1[ix0],CoverageSEMax2)+1 ## if there are no unique k-mers but there are repeated perfect matches, use the first such match to assign a provisional start
            if (SingleReadMode1==1) and ReportEnds1:
                if p1!=0:
                    CoverageE1[x1]= min(CoverageE1[x1],CoverageSEMax2)+1
                elif 0<=ix1<ulen1:
                    CoverageE1[ix1]= min(CoverageE1[ix1],CoverageSEMax2)+1
            if (SingleReadMode1 == 0) and ReportEnds1:
                if p2!=0:
                    CoverageE1[x2]= min(CoverageE1[x2],CoverageSEMax2)+1
                elif 0<=ix2<ulen1:
                    CoverageE1[ix2]= min(CoverageE1[ix2],CoverageSEMax2)+1
            if BaseByBase1:
                ip0S = ip0
                ip0A = -ip0
                ip1S = ip1+klen1-1
...
F5MTotals1=[sum( (MultiplicityD1[c11][i]*int(i) for i in MultiplicityD1[c11]) ) for c11 in NameARange1]
F5MTotals2=[sum( (MultiplicityD1[c11][i]*int(i)*int(i) for i in MultiplicityD1[c11]) ) for c11 in NameARange1]
...
if Totals2[12]==0:
    ...
    S2="{0:.2f}".format(max(0.0,((Totals2[14]*1.0)/Totals2[12]-((Totals2[13]**2*1.0)/Totals2[12]**2)))**0.5)
Totals2[13]=M2
Totals2[14]=S2
...
for c99 in NameARange1[:-1]:
    ...
    if nanolist0[12]==0:
        ...
        S0="{0:.2f}".format(max(0.0,((nanolist0[14]*1.0)/nanolist0[12]-((nanolist0[13]**2*1.0)/nanolist0[12]**2)))**0.5)
    nanolist0[13]=M0
    nanolist0[14]=S0
...
PurgeREVATempFiles(ScratchPath1)
if (ReportStarts1 and np.max(CoverageS1)==CoverageSEMax1) or (ReportEnds1 and np.max(CoverageE1)==CoverageSEMax1) :
    LogNote1('***** ',LogFile1)
    LogNote1('***** Saturation advisory I: Start or End sites for at least one position reached maximum allowed and was thus capped (CoverageSEMax='+str(CoverageSEMax1)+')',LogFile1)
    LogNote1('***** If this is a problem, rerun with CoverageSEMax set to a larger value (e.g. 65535, 1000000000)',LogFile1)
    LogNote1('***** ',LogFile1)
if np.max(CoverageK1)==MaxCoverage1:
    LogNote1('***** ',LogFile1)
    LogNote1('***** Saturation advisory II: Coverage for at least one position reached maximum and was thus capped (MaxCoverage='+str(MaxCoverage1)+')',LogFile1)
    LogNote1("***** If this is a problem, rerun with CoverageDType='uint32'",LogFile1)
    LogNote1('***** ',LogFile1)

    
LogNote1('Finished running '+' '.join(argv),LogFile1)
try:
    LogNote1(open(MyCode1,mode='rt').read(),LogFile1)
except:
    pass
