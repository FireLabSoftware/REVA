...
while ai1<len(argv):
    ...
    if not('=' in a1) and (a1.lower().endswith('dfam.hits.gz') or a1.lower().endswith('dfam.hits') or a1.lower()=='dfam' or a1.lower()=='-dfam'):
        ...
        DFAMCount1=True
        continue
    if not('=' in a1) and a1.lower().endswith('.9rh'):
        IndexFile1=a1.strip()
        continue
...
if IndexFile1=='default':
    IndexFile0='PreREVAIndex_'+RefAbbrev1+'_k'+str(klen1)+'_b'+str(BitsIndexed1)+'_wv1.9rh'
    if Debug1:
...
OutFileNameBase='PreREVA_'+Mnemonic1+'_'+RefAbbrev1+'_'+strftime("D%m%d%y_T%H%M%S",localtime())
FilterFileBase1 = 'PreREVA_'+strftime("D%m%d%y_T%H%M%S_",localtime())
...
npToPyArrayDT1 =  {np.dtype('uint8'):'B',
    np.dtype('uint16'):'H',
    np.dtype('uint32'):'I',
    np.dtype('uint64'):'L',
    np.dtype('int8'):'b',
    ...
    np.dtype('int32'):'i',
    np.dtype('int64'):'l'}
for npdt1 in list(npToPyArrayDT1.keys()):
    npToPyArrayDT1[npToPyArrayDT1[npdt1]] = npdt1
def npToPyArray1(A):
    gc.collect()
    newA = array.array(npToPyArrayDT1[A.dtype],A)
    gc.collect()
    return newA
def pyArrayToNP1(A):
    gc.collect()
    newA = np.array(A,dtype=npToPyArrayDT1[A.typecode])
    gc.collect()
    return newA
def pyArrayToList1(A):
    gc.collect()
    newA = list(A)
    gc.collect()
    return newA
...
def pyListToNP1(A):
    ...
    if m<2**16:
        return np.array(A,dtype='uint16')
    if m<2**32:
        return np.array(A,dtype='uint32')
    if m<2**64:
        return np.array(A,dtype='uint64')
    else:
        return('values >= 2**64 in pyListToNP1 call')
    gc.collect()
xDumpSeg = 100000000
def xDump1(f,A):
    
    A = npToPyArray1(A)
    pp=cPickle.Pickler(f)
    pp.dump(A.typecode)
    pp.dump(len(A))
    for w in range(len(A)//xDumpSeg): 
        A[w*xDumpSeg:(w+1)*xDumpSeg].tofile(f)
    A[(len(A)//xDumpSeg)*xDumpSeg:].tofile(f)
    return None
def xLoad1(f):
    pp=cPickle.Unpickler(f)
    myType = pp.load()
    myCount = pp.load()
    A = array.array(myType)
    for w in range(myCount//xDumpSeg):
        A.fromfile(f,xDumpSeg)        
    A.fromfile(f,myCount%xDumpSeg)
    return A
dtype1=np.uint16
if klen1>8:
    dtype1=np.uint32
...
if RefSeqFile1.endswith('.gz'):
    if version.startswith('2.'):
        F0=gzip.open(RefSeqFile1,mode='rU')
    else:
...
try:
    ...
    ulenR1=pp1.load()
    NameA1=pp1.load()
    Sort_P1dedup = xLoad1(pf1)
    Sort_I1dedup = xLoad1(pf1)
    FastIndex1 = xLoad1(pf1)
    Sort_V1dedup = xLoad1(pf1)
    pf1.close()
    LogClosingFile1(pf1)
    ...
    pp1.dump(NameA1)
    Sort_P1dedup = xDump1(pf1,Sort_P1dedup)
    Sort_I1dedup = xDump1(pf1,Sort_I1dedup)
    FastIndex1 = xDump1(pf1,FastIndex1)
    Sort_V1dedup = xDump1(pf1,Sort_V1dedup)
    pf1.close()
    LogClosingFile1(pf1)
...
for fnn1 in FiLR1+FiLR2:
    for myn1,myc1 in enumerate(os.path.basename(fnn1)):
        if len(NameConsensus1)<myn1+1:
            NameConsensus1.append(myc1)
        elif NameConsensus1[myn1]!=myc1:
            NameConsensus1 = NameConsensus1[:myn1]
            break
    if fnn1.lower().endswith('fasta') or fnn1.lower().endswith('fasta.gz'):
        FastAProject0 = True
NameConsensus1 = ''.join(NameConsensus1)
if NameConsensus1.lower().endswith('.gz'):
    NameConsensus1=NameConsensus1[:-3]
if NameConsensus1.lower().endswith('.fasta') or NameConsensus1.lower().endswith('.fastq'):
    NameConsensus1=NameConsensus1[:-6]
if NameConsensus1.lower().endswith('_r'):
    NameConsensus1=NameConsensus1[:-2]
if NameConsensus1.lower().endswith('_'):
    NameConsensus1=NameConsensus1[:-1]
if ''.join(FiLR1):
    SingleReadMode0+=1
    if FastAProject0:
        FastF1 = FilterFileBase1+NameConsensus1+'_R1.fasta.gz'
    else:
        FastF1 = FilterFileBase1+NameConsensus1+'_R1.fastq.gz'
    F11 = gzip.open(FastF1, mode='wt')    
if ''.join(FiLR2):
    SingleReadMode0+=2
    if FastAProject0:
        FastF2 = FilterFileBase1+NameConsensus1+'_R2.fasta.gz'
    else:
        FastF2 = FilterFileBase1+NameConsensus1+'_R2.fastq.gz'
    F12 = gzip.open(FastF2, mode='wt')    
    ...
    for GFF11 in GFF1:
        ...
        for L0 in GFFile1:
            ...
            if (ch in NameD1) and ((L1[2] in Feature1D) or Feature1All):
                ...
                Dfo1.append(L1[6])
        GFFile1.close()
    F7=open("FeatureSummary_"+OutFileNameBase+'.tdv',mode='wt')
    LogOpeningFile1(F7)
...
if BinByBin1:
    ...
    F10=open("BinByBinReadCountSummary_"+OutFileNameBase+'.tdv',mode='wt')
    LogOpeningFile1(F10)
...
if ChromosomeByChromosome1:
    ...
    F11=open("ChromosomeByChromosomeReadCountSummary_"+OutFileNameBase+'.tdv',mode='wt')
    LogOpeningFile1(F11)
...
Sort_I1dedup = pyArrayToNP1(Sort_I1dedup)
Sort_P1dedup = pyArrayToNP1(Sort_P1dedup)
...
if FirstKTable1:
    ...
    F13=open("FirstKTable_"+OutFileNameBase+'.tdv',mode='wt')
    LogOpeningFile1(F13)
