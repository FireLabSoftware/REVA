...
## REVA Output JunctionParser version ya02 09-10-20
##
##   call syntax
##          python REVAJunctionParser<ver>.py <REVA_JunctionFile>
##   Optional parameters that can be set in the command line are
##      ReportDeletions=True (or =False)
##      ReportCircles=True (or =False)
##      IgnoreNum=<Number>  (Number of bases to ignore at the beginning of each read)
##      BarcodeFile = A file with barcodes defines (one per line, with the line format <Name> <BarcodeSequence>
##      R1BarcodeColumn = Allows user to specify which column Read1 5' barcode/buffer is in (will obtain from headers otherwise)
##      R2BarcodeColumn = Allows user to specify which column Read2 5' barcode/buffer is in (will obtain from headers otherwise)
##
...
ReportCircles1 = True ## Report Putative Circles?
...
BarcodeD1 = {}  ## Keys are barcodes, values are mnenomics
r1b5 = -2 ## Column number for R1Buffer (if included).  Will also grab this from headers if present
r2b5 = -1

def isSeq1(s):
    ## returns true if s is a pure sequence of longer than 1 base (>1 deals with the possibility of AGCT being labels)
    if len(s)>1 and len(s.upper().replace('A','').replace('G','').replace('C','').replace('T',''))==0:
        return True
    return False
...
while ai1<len(argv):
    ...
    if (a1.strip('-').startswith('help')):
        mycode1 = open(argv[0]).read().splitlines()
        for h1 in mycode1:
            if h1.startswith('##'):
                print(h1[2:])
            if h1.startswith('import') or h1.startswith('## More Details In Script'):
                print(' ')
                break
        exit()
    if not('=') in a1 and not(a1.startswith('-')):
        if len(MetaName1)<100:
            MetaName1 += a1.split('JunctionEvents_REVA_')[-1].replace('*','').split('.')[0]
        else:
    ...
    if a11.startswith('ignore'):
        IgnoreNum1=int(a22)
    elif a11.startswith('reportdel') or a11.startswith('deletion'):
        if a22.lower().startswith('f'):
            ReportDeletions1=False
        else:
            ReportDeletions1=True
    elif a11.startswith('reportcir') or a11.startswith('circl'):
        if a22.lower().startswith('f'):
            ReportCircles1=False
        else:
            ReportCircles1=True
    elif a11.startswith('barcode'):
        BarcodeList1=open(a22,mode='rt').read().splitlines()
        for L0 in BarcodeList1:
            L1 = L0.split()
            if len(L1)>1:
                if isSeq1(L1[1]):
                    BarcodeD1[L1[1]]=L1[0]
                    ...
                    BarcodeD1[L1[0]]=L1[1]
    elif a11.startswith('r1b'):
        r1b5 = int(a22)
    elif a11.startswith('r2b'):
        r2b5 = int(a22)
...
if not(FL1):
    ...
    raise(SystemError)
FGood1 = open('GoodJunctions_'+MetaName1+now1+'.tdv', mode='wt')
FBad1 = open('QuestionableJunctions_'+MetaName1+now1+'.tdv', mode='wt')
def antisense(s):
...
class A3:
    def __init__(self,s1,s2,s3,JLen = 25,MakeTextDisplay=True,SenseStrand2=True,SenseStrand3=True):
        self.aF12 = A2(s1,s2,ReverseAlign=False,LateFavor=not(SenseStrand2))  ## forward alignment, sequence s2
        self.aR13 = A2(s1,s3,ReverseAlign=True,LateFavor=SenseStrand3)
        self.JLen = JLen
        ...
        if self.pivot>=0:
            ...
            self.JScore = list(map(sum,zip(self.aF12.nV,self.aR13.nV)))  ## differential value of including position j in left arm
...
if BarcodeD1:
    MetaD7 = {}  ## Dictionary indices are rearrangments
             ## Values are Dictionaries with Key being barcode combo and value being count

HeadersWritten1 = False
TypeD0={'1s1s':0,'1s1a':0,'1a1s':0,'1a1a':0,'2s2s':0,'2s2a':0,'2a2s':0,'2a2a':0} 
    ...
    for g1,L0 in enumerate(F1):
        ...
        if not(HeadersWritten1) and len(L1)>20:
            ...
            if L1[-1].startswith('<!'):
                ...
                L0 = '\t'.join(L1[:-1])
            if BarcodeD1:
                if 'R1Buffer5' in L1 and r1b5==-2:
                    r1b5 = L1.index('R1Buffer5')
                if 'R2Buffer5' in L1 and r2b5==-1:
                    r2b5 = L1.index('R2Buffer5')
            FGood1.write(L0.strip()+'\t')
            FGood1.write(GoodyHeader1)
            FBad1.write(L0.strip()+'\t')
            FBad1.write(GoodyHeader1)
            if descriptor1:
                FGood1.write('\t'+descriptor1)
                FBad1.write('\t'+descriptor1)
            FGood1.write('\r')
            FBad1.write('\r')
            HeadersWritten1 = True            
        SkipThisLine1 = True
        if len(L1)>12 and ((ReportDeletions1 and ('Deletion_Junction' in L1[0])) or (ReportCircles1 and ('Circle_Junction' in L1[0]))):
            SkipThisLine1 = False
        if SkipThisLine1:
            continue
        OutFile=FBad1
...
##            SampleID1 = L1[30].split('#')[0].split(':')[-1]
        SampleID1 = L1[-1].split('#')[0].split(':')[-1]
        if BarcodeD1:
            Barcode11 = 'NoB1'
            for b11 in BarcodeD1:
                if L1[r1b5].startswith(b11):
                    Barcode11=BarcodeD1[b11]
                    break
            Barcode22 = 'NoB2'
            for b11 in BarcodeD1:
                if L1[r2b5].startswith(b11):
                    Barcode22=BarcodeD1[b11]
                    break
            Barcode1 = Barcode11+'+'+Barcode22
        if ('1s1s' in L1[0]) or ('1a1a' in L1[0]) or  ('1s1a' in L1[0]) or ('1a1s' in L1[0]):
            Sense0 = ('1s1s' in L1[0]) or ('1s1a' in L1[0])
            Sense1 = ('1s1s' in L1[0]) or ('1a1s' in L1[0])
            if Sense0:
                Moniker1 = '1s'
            else:
                Moniker1 = '1a'
            ...
            s2 = L1[12][IgnoreNum1:]
            s3 = L1[16][IgnoreNum1:]
            a123 = A3(s1,s2,s3,SenseStrand2=Sense0, SenseStrand3=Sense1)
            ...
            if Sense0:
                ...
                PreciseEventStartLo1 = -int(L1[10])-a123.s2JMax+1
                PreciseEventStartHi1 = -int(L1[10])-a123.s2JMin+1
            ...
            if Sense1:
                ...
                PreciseEventEndLo1 = -int(L1[14])-a123.s3JMax+1
                PreciseEventEndHi1 = -int(L1[14])-a123.s3JMin+1
                JD5p = antisense(a123.s2J3p)
                JD3p = antisense(a123.s2J5p)
            DupSeq = a123.JDup
            InsSeq = a123.JIns
            if OutFile==FGood1:
                Index1 = ((L1[9],PreciseEventStartLo1,PreciseEventStartHi1,Sense0),
                          (L1[13],PreciseEventEndLo1,PreciseEventEndHi1,Sense1),
                          InsSeq)
                if not Index1 in MetaD1:
                    ...
                    if BarcodeD1:
                        MetaD7[Index1] = {}
                if BarcodeD1:
                    if not(Barcode1 in MetaD7[Index1]):
                        MetaD7[Index1][Barcode1] = 0
                    MetaD7[Index1][Barcode1] += 1
                MetaD4[Index1].append(DupSeq)
                MetaD5[Index1].append(a123.pivot)
                Index2 = '_'.join((Moniker1,str(a123.s2JMin),SampleID1))
            ...
            Goodies = [ PreciseEventStartLo1,
                        ...
                        a123.A]
            if BarcodeD1:
                Goodies.append(Barcode1)
            L0 = L0.strip() + '\t' + '\t'.join(map(str,Goodies))
            OutFile.write(L0)
            OutFile.write('\r')


        if ('2s2s' in L1[0]) or ('2a2a' in L1[0]) or  ('2s2a' in L1[0]) or ('2a2s' in L1[0]):
            Sense0 = ('2s2s' in L1[0]) or ('2s2a' in L1[0])
            Sense1 = ('2s2s' in L1[0]) or ('2a2s' in L1[0])
            if Sense0:
                Moniker1 = '2s'
            else:
                Moniker1 = '2a'
            ...
            if Sense0:
                ...
                PreciseEventStartLo1 = -int(L1[21])-a123.s2JMax+1
                PreciseEventStartHi1 = -int(L1[21])-a123.s2JMin+1
                JU5p = antisense(a123.s3J3p)
                JU3p = antisense(a123.s3J5p)
            if Sense1:
                ...
                PreciseEventEndLo1 = -int(L1[25])-a123.s3JMax+1
                PreciseEventEndHi1 = -int(L1[25])-a123.s3JMin+1
                JD5p = antisense(a123.s2J3p)
                JD3p = antisense(a123.s2J5p)
            DupSeq = a123.JDup
            InsSeq = a123.JIns
            if OutFile==FGood1:
                Index1 = ((L1[20],PreciseEventStartLo1,PreciseEventStartHi1,Sense0),
                          (L1[24],PreciseEventEndLo1,PreciseEventEndHi1,Sense1),
                          InsSeq)
                if not Index1 in MetaD1:
                    MetaD1[Index1] = {}
                    MetaD2[Index1] = []
...
FGood1.close()
FBad1.close()
MetaOut1 = open('AggregateJunctionSummary_'+MetaName1+now1+'.tdv',mode='wt')
MetaOutHeader1 = 'Ups_Chr\tUps_Lo\tUps_Hi\tUps_Str\tDwn_Chr\tDwn_Lo\tDwn_Hi\tDwn_Str\tInsSeq\tDupSeq\tPivot_Distance\tDup_Qual\tUps_5p\tUps_3p\tDwn_5p\tDwn_3p\tAlign_Types\tAlign_Strings\tAlign_Correlation\tAlign_Aggregate\tMatch\tMismatch\tLeftInconsistent\tRightInconsistent\tLeftSupportive\tRightSupportive\tInstances\tUniqueSettings\tRearrangment\tUCSC_Link(FirstInstance)'
if BarcodeD1:
    MetaOutHeader1 += '\tBarcodes'
if descriptor1:
    MetaOutHeader1 += '\t'+descriptor1
MetaOutHeader1 += '\r'
MetaOut1.write(MetaOutHeader1)
for Index1 in sorted(MetaD1.keys()):
    del1 = Index1[0][1]-Index1[1][1]
    if Index1[0][0]==Index1[1][0]:
        if Index1[0][3] and Index1[1][3]:
            if del1>=0:
                Mnemonic1 = 'DelS_'+str(del1)
            else:
                Mnemonic1 = 'CirS_'+str(-del1)            
        elif not(Index1[0][3]) and not(Index1[1][3]):
            if del1<=0:
                Mnemonic1 = 'DelA_'+str(-del1)
            else:
                Mnemonic1 = 'CirA_'+str(del1)
        elif Index1[0][3] and not(Index1[1][3]):
            if del1>=0:
                Mnemonic1 = 'InvLS_'+str(del1)
            else:
                Mnemonic1 = 'InvLA_'+str(-del1)            
        elif not(Index1[0][3]) and Index1[1][3]:
            if del1>=0:
                Mnemonic1 = 'DelRS_'+str(del1)
            else:
                Mnemonic1 = 'CirRA_'+str(-del1)
    else:
        if Index1[0][3] and Index1[1][3]:
            Mnemonic1 = 'TrnSS_'+str(del1)
        elif Index1[0][3] and not(Index1[1][3]):
            Mnemonic1 = 'TrnSA_'+str(del1)
        elif not(Index1[0][3]) and Index1[1][3]:
            Mnemonic1 = 'TrnAS_'+str(del1)
        elif not(Index1[0][3]) and not(Index1[1][3]):
            Mnemonic1 = 'TrnAA_'+str(del1)
    ...
    MetaMetaD1 = {}
    for key1 in MetaD1[Index1]:
        key2 = '_'.join(key1.split('Read=')[0].split('_')[:4])
        if not key2 in MetaMetaD1:
            MetaMetaD1[key2] = 0
        MetaMetaD1[key2] += 1
    SettingList1 = ', '.join(sorted(str(MetaMetaD1).replace("'","")[1:-1].split(', ')))    
    HitsUnique1 = len(MetaMetaD1)
    MetaOut1.write('\t'.join(map(sensestring,Index1[0]+Index1[1]))+'\t'+Index1[2]+'\t')
    MetaOut1.write(aov1+'\t'+ ("%.2f" % Pivot1) +'\t'+("%.3f" % aoq1) +'\t')
    MetaOut1.write('\t'.join(map(str,MetaD3[Index1]))+'\t')
    MetaOut1.write(SettingList1+'\t')
    ...
    MetaOut1.write(MetaD6[Index1])
    if BarcodeD1:
        MetaOut1.write('\t'+str(MetaD7[Index1]).replace('"','').replace('{','').replace('}','').replace(': ',':'))
    MetaOut1.write('\r')
MetaOut1.close()

##tetritisK1 = 12
##tetritisEndFuzzy = 4
##tetritis=open('illuminatetritis.fa',mode='rt').read()
##
